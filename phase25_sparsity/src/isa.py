...
OP_WGMMA  = 0x38  # warp-group MMA using shared memory tiles (Phase 14)
OP_CPYASYNC = 0x39  # async copy: rs1=src, rs2=dst, imm=size (Phase 17)
OP_RECORD   = 0x3A  # record event: imm=event_id (Phase 17)
OP_WAIT     = 0x3B  # wait event: imm=event_id (Phase 17)
OP_TMA_LOAD = 0x3C  # TMA async load: desc=rd, smem=rs1, start_addr=imm (Phase 20)
OP_TMA_STORE= 0x3D  # TMA async store (Phase 20)
OP_FP8MMA  = 0x3E  # FP8 tensor MMA (Phase 21)
OP_CNVT    = 0x3F  # convert precision: rd = convert(rs1, fmt) (Phase 21): desc=rd, smem=rs1, start_addr=imm (Phase 20)

# --- 内存指令 (Phase 5) ---
OP_SHLD = 0x31  # rd = shared_mem[imm]
OP_SHST = 0x32  # shared_mem[imm] = rs1

# --- Async Pipeline (Phase 22) ---
OP_ABAR   = 0x40  # async barrier: increment counter, check if all arrived
OP_PCOMMIT = 0x41  # pipeline commit: signal stage completion

# --- Independent Thread Scheduling (Phase 23) ---
OP_TBRANCH = 0x42  # thread-level branch: each thread independently branches
OP_TRECONV = 0x43  # reconvergence point marker

# --- Thread Block Cluster (Phase 24) ---
OP_DSM_LD = 0x44  # load from distributed shared memory
OP_DSM_ST = 0x45  # store to distributed shared memory
OP_CBAR   = 0x46  # cluster barrier

# --- Structured Sparsity (Phase 25) ---
OP_SPMMA   = 0x47  # sparse MMA: skip zero multiplies
OP_SPACK   = 0x48  # pack dense -> sparse 2:4 format
OP_SUNPACK = 0x49  # unpack sparse -> dense

# --- Tensor Core (Phase 9) ---
OP_MMA = 0x4A  # rd = rs1.hi*rs2.hi + rs1.lo*rs2.lo + rs3
...
OPCODE_NAMES = {
    ...
    OP_WGMMA: "WGMMA",
    OP_CPYASYNC: "CPYASYNC", OP_RECORD: "RECORD", OP_WAIT: "WAIT",
    OP_TMA_LOAD: "TMA_LOAD", OP_TMA_STORE: "TMA_STORE",
    OP_FP8MMA: "FP8MMA", OP_CNVT: "CNVT",
    OP_ABAR: "ABAR", OP_PCOMMIT: "PCOMMIT",
    OP_TBRANCH: "TBRANCH", OP_TRECONV: "TRECONV",
    OP_DSM_LD: "DSM_LD", OP_DSM_ST: "DSM_ST", OP_CBAR: "CBAR",
    OP_SPMMA: "SPMMA", OP_SPACK: "SPACK", OP_SUNPACK: "SUNPACK",
}
