...
def _assemble_line(line: str, labels: dict, current_pc: int) -> int:
    ...
    # ---- 共享内存 ----
    if mnemonic == 'SHLD':
        return encode_itype(OP_SHLD, _parse_reg(parts[1]), _parse_mem_addr(parts[2]))
    if mnemonic == 'SHST':
        return encode_stype(OP_SHST, _parse_reg(parts[1]), _parse_mem_addr(parts[2]))
    ...
    if mnemonic == 'MMA':
        ...
        return encode_mma(rd, rs1, rs2, rs3)
